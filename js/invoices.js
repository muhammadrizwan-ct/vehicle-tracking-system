...
function displayVendorInvoicesTable(invoices) {
    ...
    if (permissions.canManageInvoices && canDeleteInvoices) {
        html += '<th>Actions</th>';
    } else if (permissions.canManageInvoices && canGenerateInvoices) {
        html += '<th>Actions</th>';
    }
    ...
    invoices.forEach(inv => {
        ...
        if (permissions.canManageInvoices && (canGenerateInvoices || canDeleteInvoices)) {
            ...
            if (canGenerateInvoices) {
                html += `<button class="btn btn-sm btn-primary" onclick="showEditVendorInvoiceModal('${safeNo}')" title="Edit Invoice" style="width: 28px; height: 28px; padding: 0; margin-right: 4px;"><i class="fas fa-edit"></i></button>`;
            }
            if (canDeleteInvoices) {
                html += `<button class="btn btn-sm" onclick="deleteVendorInvoice('${safeNo}')" title="Delete Invoice" style="background: var(--danger); color: white; width: 28px; height: 28px; padding: 0;"><i class="fas fa-trash"></i></button>`;
            }
            html += '</td>';
        }
...
window.showEditVendorInvoiceModal = showEditVendorInvoiceModal;
window.updateVendorInvoice = updateVendorInvoice;
